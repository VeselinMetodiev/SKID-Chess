...
namespace eval pgn {
  ...
  # Create a procedure to show an alert
  proc showAlert {message} {
    tk_messageBox -icon info -title "Alert" -message $message
  }
  ################################################################################
  #
  ################################################################################
  ...
  proc Refresh { {pgnNeedsUpdate 0} } {
    ...
    if {$pgnNeedsUpdate} {
      ...
      set replacementMap {Q Д N K B O R T}  ;# Replace 'o' with 'O' and 'l' with 'L'

      set pgnStr [string map $replacementMap $pgnStr]
